...
if not(isfolder("plots/impedances"))
    mkdir("plots/impedances")
end
...
saving = 1; %salvare =1, non salvare=0

for ii=1:numEval

%     solnum = ii+44; % isotropic damping
    solnum = ii + 81; % rayleigh damping
    ...
    label = model.sol(sol).label();
    mytitle = label.toCharArray';
    mytitle = mytitle(1:16);
    if (plotting)
        figure('Renderer', 'painters', 'Position', [100 100 1000 500]);
        plot(f, abs(Zin(ii).imp), LineWidth=2);
%         mytitle = model.sol(sol).label();
        title('Input Impedance',mytitle, fontSize=20);
        ...
        if (saving)
            ...
            saveas(gcf, [".\plots\impedances\"+filename+".png"]);
        end
    end
end
...
% PLOTTA LA PARTE IMMAGINARIA, SE VUOI LA PARTE REALE INVERTI LE RIGHE
% COMMENTATE 
...
% PARTE REALE
    ...
    grid minor
    title(["Detuning Curves on Time Decay- "+notes(ii)], ["f_0="+num2str(sg(ii))+"Hz"], FontSize=15)
    xlabel('\epsilon', FontSize=15); ylabel('\Re(\alpha)', FontSize=15)
    xlim([min(epsi), max(epsi)])
    saveas(gcf, ["./plots/Detuning_"+notes(ii)+".png"]);
    ...
    string(ii).T60_1 = -log(1e-3)*string(ii).tau1;
    string(ii).T60_2 = -log(1e-3)*string(ii).tau2;

    % trova il T60 della corda totale per via numerica
